...
# user, so a second daemon for a second screen refuses to start and that screen
# ends up with no sound card at all. Sinks are cheap; daemons are not something
# this can have more than one of.
...
start() {
  ...
  # whichever runtime directory the caller happens to have, so a daemon
  # started here and a client started by an exec look in two different places
  # and the client reports "connection refused" beside a running daemon.
  # Everything reaches it through PULSE_SERVER instead.
...
control() {
  ...
  # write would leave the older token in the file. That would let the replaced
  # holder end the takeover that replaced it, and refuse the current holder.
...
release() {
  ...
  # must not be endable by whoever it replaced. `--force` is the deliberate
  # way past it, for a caller that has decided the person is finished.
  want="${3:-}"
