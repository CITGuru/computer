...
# **One daemon for the box, one sink per screen.** PulseAudio is a singleton
# per user, so a second daemon for a second screen refuses to start and that
# screen ends up with no sound card at all. Sinks are cheap; daemons are not
# something this can have more than one of.
...
# How many browsers are actually on a viewer, rather than whether the server
# is up.
#
# websockify holds a connection to x11vnc only while a websocket client is
...
start() {
  ...
  # A sound card for this screen, where one is installed. The sink goes
  # nowhere: nothing plays out of a box, and a recorder can still capture it.
  #
  # **The socket is named, not defaulted.** PulseAudio puts its socket under
  # whichever runtime directory the caller happens to have, so a daemon started
  # here and a client started by an exec look in two different places and the
  # client reports "connection refused" beside a running daemon. Everything
  # reaches it through PULSE_SERVER instead.
...
control() {
  ...
  # **Recorded even here**: a second takeover replaces the first, and skipping
  # the write would leave the older token in the file — where it would let the
  # replaced holder end the takeover that replaced it, and refuse the holder
...
release() {
  # **A stale release is refused rather than obeyed.** A takeover that was
  # replaced must not be endable by whoever it replaced: that would take the
  # keyboard from the person driving now, and neither of them would be told
  # why. `--force` is the deliberate way past it, for a caller that has decided
