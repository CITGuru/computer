...
Serde and a digest. Everything that reads these depends on this crate — a server, a client, a CLI, and the engine through `Builder::from_spec` — so anything heavier is paid for by all of them.

In particular this crate must never depend on `computer`, because that edge is going to run the other way. The name is the rule: if it is not a plain data type, it does not go in here.
