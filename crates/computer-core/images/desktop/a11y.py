#!/usr/bin/env python3
# Queries, not ids: an AT-SPI path dies when the widget behind it is rebuilt.
import json
import os
import sys
...
# A full walk is thousands of D-Bus round trips, so a tree read answers the window.
DEPTH_DEFAULT = 4

# A search reads the whole application; VISIT_CEILING bounds it instead.
DEPTH_ANY = 64

# The ceiling stops a search of a text editor's document from taking a minute.
FOUND_DEFAULT = 20
VISIT_CEILING = 4000

# Pixels between a label and the widget it names.
LABEL_REACH = 220

# A query usually names one of these, so they rank above an enclosing container.
ACTIONABLE = {
    "push button",
    "toggle button",
    "radio button",
...
def labels_of(node):
    """A GTK entry's own name is empty; its words live in a sibling label."""
    words = []
    try:
...
def names_widget(label, widget):
    """Geometry, because most applications publish no `LABELLED_BY` relation."""
...
def active_first(apps):
    """Two windows can each hold an OK button; the active one is meant."""
...
def ranked(query, role, exact, app_filter, limit):
    ...
    for app_index, app in active_first(list(applications(app_filter))):
        ...
        # One walk: every node is a D-Bus round trip, and the label pass needs the same nodes.
        seen = []
        ...
        for node, node_id, node_role, extents in seen:
            ...
            if not role or node_role == role:
                # The label itself ranks behind the field it names.
...
def main():
    ...
    if verb == "invoke":
        ...
        # Action 0 unless named: GTK calls it "click" and Qt "Press".
        index = 0
        if action_name:
